...
export class PostgreSQLIntrospector extends BaseIntrospector {
  ...
  async getTableStatistics(
    ...
    const tableStatsResult = await this.adapter.query(`
      ...
      WHERE schemaname = ${quoteStringLiteral(schema)} AND relname = ${quoteStringLiteral(table)}
    `);
  ...
  private buildOptimizedColumnStatsQuery(
    ...
    const rawStatsSelects = columns
      .map((column, index) => {
        ...
        const quotedColumn = quoteIdentifier(columnName, "postgresql");
...
ORDER BY s.column_name`;
  ...
  override async getFullIntrospection(options?: {
    ...
    const columnsWithStats = await this.attachColumnStatisticsPostgreSQL(
      tables,
      columns,
    );
