...
RETURNS: URL, expiry, exact expected ETag, filename, MIME type, byte size, and mandatory request headers.`,
  ...
  finalizeTableArtifact: `Verify an uploaded canonical table artifact and commit immutable same-session metadata and lineage. The server independently streams Azure bytes and validates SHA-256, ETag, schema, rows, columns, cells, and size. Pass only the bounded receipt from emit_table; never pass rows. An already-committed same-session create_query_table_artifact receipt is returned unchanged as an idempotent compatibility response; it is never rewritten.`,
